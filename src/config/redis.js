...
function buildRedisConfig(env) {
    ...
    // ✅ FIXED: Check for both falsy value AND empty/whitespace string
    // Previously: if (env.REDIS_PASSWORD) — this passed empty string "" through
    // which caused Redis to send a password to a server that doesn't require one,
    // triggering the warning: "user does not require a password, but a password was supplied"
