...
export async function getCountryInfo(req, res) {
    ...
    if (!data) {
        return sendError(res, {
            ...
            code: 'NOT_FOUND',
            message: `Could not find country: ${name}`,
        });
    }
...
export async function getWorldTime(req, res) {
    ...
    if (!data) {
        return sendError(res, {
            statusCode: HTTP.NOT_FOUND,
            code: 'NOT_FOUND',
            message: `Could not fetch time for timezone: ${timezone}`,
        });
    }
...
export async function checkHoliday(req, res) {
    ...
    if (!country || !date) {
        return sendError(res, {
            statusCode: HTTP.BAD_REQUEST,
            code: 'VALIDATION_ERROR',
            message: 'Both country (2-letter code) and date (YYYY-MM-DD) are required.',
        });
    }
...
export async function getPublicHolidays(req, res) {
    ...
    if (!data) {
        return sendError(res, {
            statusCode: HTTP.NOT_FOUND,
            code: 'NOT_FOUND',
            message: `No holiday data found for ${country}/${year}.`,
        });
    }
...
export async function getIPInfo(req, res) {
    ...
    if (!data) {
        return sendError(res, {
            statusCode: HTTP.NOT_FOUND,
            code: 'NOT_FOUND',
            message: `Could not retrieve info for IP: ${ip}`,
        });
    }
...
export async function getExchangeRate(req, res) {
    ...
    if (!from || !to) {
        return sendError(res, {
            statusCode: HTTP.BAD_REQUEST,
            code: 'VALIDATION_ERROR',
            message: 'Both "from" and "to" currency codes are required (e.g., USD, EUR).',
        });
    }
    ...
    if (!/^[A-Za-z]{3}$/.test(from) || !/^[A-Za-z]{3}$/.test(to)) {
        return sendError(res, {
            statusCode: HTTP.BAD_REQUEST,
            code: 'VALIDATION_ERROR',
            message: 'Currency codes must be 3-letter ISO codes (e.g., USD, EUR, GBP).',
        });
    }
    ...
    if (!data) {
        return sendError(res, {
            statusCode: HTTP.NOT_FOUND,
            code: 'NOT_FOUND',
            message: `Could not fetch exchange rate for ${from.toUpperCase()} → ${to.toUpperCase()}.`,
        });
    }
...
export async function getExchangeRates(req, res) {
    ...
    if (!base) {
        return sendError(res, {
            statusCode: HTTP.BAD_REQUEST,
            code: 'VALIDATION_ERROR',
            message: 'Base currency code is required (e.g., USD).',
        });
    }
    ...
    if (!/^[A-Za-z]{3}$/.test(base)) {
        return sendError(res, {
            statusCode: HTTP.BAD_REQUEST,
            code: 'VALIDATION_ERROR',
            message: 'Base currency must be a 3-letter ISO code (e.g., USD).',
        });
    }
    ...
    if (!data) {
        return sendError(res, {
            statusCode: HTTP.NOT_FOUND,
            code: 'NOT_FOUND',
            message: `Could not fetch exchange rates for base currency: ${base.toUpperCase()}.`,
        });
    }
